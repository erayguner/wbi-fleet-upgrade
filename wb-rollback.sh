...
log_debug() { [[ "$VERBOSE" == "true" ]] && echo -e "${CYAN}[DEBUG]${NC} $1"; return 0; }
log_step()  { echo -e "${BLUE}${BOLD}==> $1${NC}"; }

check_command() {
    command -v "$1" &>/dev/null || { log_error "$1 is not installed or not in PATH"; return 1; }
}
...
find_python() {
  ...
  if [[ -n "$PYTHON_CMD" ]]; then
    command -v "$PYTHON_CMD" &>/dev/null && { echo "$PYTHON_CMD"; return 0; }
...
setup_venv() {
  ...
  "$python_cmd" -m venv "$VENV_DIR" || { log_error "Failed to create venv"; return 1; }
...
install_dependencies() {
  ...
  for pkg in "${REQUIRED_PACKAGES[@]}"; do
    $PYTHON_CMD -c "import pkg_resources; pkg_resources.require('$pkg')" &>/dev/null || { needs_install=true; break; }
  done
  ...
  if [[ -f "$REQUIREMENTS_FILE" ]]; then
    $pip_cmd install -r "$REQUIREMENTS_FILE" --quiet && { log_info "✓ Dependencies installed"; return 0; }
  fi

  for pkg in "${REQUIRED_PACKAGES[@]}"; do
    $pip_cmd install "$pkg" --quiet 2>/dev/null || { log_error "Failed to install: $pkg"; return 1; }
  done
...
validate_gcp_auth() {
    ...
    check_command gcloud || { log_error "Google Cloud SDK required: https://cloud.google.com/sdk/docs/install"; return 1; }
    ...
    if [[ -z "${GOOGLE_APPLICATION_CREDENTIALS:-}" && ! -f "$HOME/.config/gcloud/application_default_credentials.json" ]]; then
        ...
        gcloud auth application-default login --quiet 2>/dev/null || { log_error "Failed to setup ADC"; return 1; }
    fi
...
validate_project() {
    ...
    gcloud projects describe "$PROJECT_ID" &>/dev/null || { log_error "Cannot access project: $PROJECT_ID"; return 1; }
...
while [[ $# -gt 0 ]]; do
    ...
    --skip-venv-check)      SKIP_VENV_CHECK="true"; shift ;;
...
[[ -z "$PROJECT_ID" ]] && { log_error "Project ID required (--project or GCP_PROJECT_ID)"; show_usage; exit 1; }
[[ -z "$LOCATIONS" ]] && { log_error "Locations required (--locations or LOCATIONS)"; show_usage; exit 1; }
[[ ! -f "$PYTHON_SCRIPT_ROLLBACK" ]] && { log_error "Python script not found: $PYTHON_SCRIPT_ROLLBACK"; exit 1; }
...
if [[ -f "$PYTHON_SCRIPT_ROLLBACK" ]]; then
  ...
  if "$PYTHON_CMD" "$PYTHON_SCRIPT_ROLLBACK" "${ARGS[@]}"; then
    echo ""; echo -e "${GREEN}${BOLD}========================================================"; echo " Rollback process completed successfully"; echo "========================================================${NC}"; exit 0
  else
    EXIT_CODE=$?; echo ""; echo -e "${RED}${BOLD}========================================================"; echo " Rollback process failed (exit code: $EXIT_CODE)"; echo "========================================================${NC}"; log_error "Check workbench-rollback.log and rollback-report-*.json"; exit $EXIT_CODE
  fi
fi
